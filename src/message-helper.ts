...
export class Rule {
  ...
  private removeFixups(): boolean {
    ...
    if (found) {
      core.info(
        "'fixup!' found in the commit header.\nPlease remove it before merge 🙂"
      );
  ...
  private checkHeader(): boolean {
    ...
    if (header == null) {
      ...
      this.errors.add(
        "Header does not follow the format : " + "<type>(<scope>): <subject>"
      );
    } else {
      const type = header[1];
    ...
    if (this.commit.header.length > this.config.maxHeaderLength) {
      ...
      this.errors
        .add(`Length of header cannot be more than ${this.config.maxHeaderLength}.
If required, change the value of input parameter max-header-length in your .yml file`);
  ...
  public check(): boolean {
    ...
    if (!ok) throw this.errors.getCollectiveError();
